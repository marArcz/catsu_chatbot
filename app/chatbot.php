...
class Chatbot
{
    ...
    {
        $schoolYears = [];
        $query = $this->pdo->prepare("SELECT * FROM enrollments WHERE student_id_no = ? GROUP BY year_start");
    ...
    function getGrades(string $user_query): array
    {
        ...
        if (stristr($user_query, 'semester') && Session::hasSession($schoolYearSession)) {
            ...
            $get_enrolled_courses = $this->pdo->prepare("SELECT enrolled_courses.*,courses.name as course_name,courses.code as course_code FROM enrolled_courses INNER JOIN courses ON enrolled_courses.course_code = courses.code WHERE enrolled_courses.enrollment_id IN (SELECT id FROM enrollments WHERE year_start = ? AND year_end = ? AND semester = ? AND student_id_no=?)");
            $get_enrolled_courses->execute([$yearStart, $yearEnd, $semester, $this->user['student_id_no']]);
            ...
            if ($enrolled_courses) {
                ...
                foreach ($enrolled_courses as $key => $enrolled_course) {
                    ...
                    $get_grade = $this->pdo->prepare('SELECT * FROM grades WHERE course_code = ? AND enrollment_id = ?');
                    ...
                    if ($grade) {
                        ...
                        $getCourseInfo = $this->pdo->prepare("SELECT * FROM courses WHERE code = ?");
                        $getCourseInfo->execute([$course_code]);
                        $courseInfo = $getCourseInfo->fetch();

                        $sum += $courseInfo['unit'] * $grade;
                    } else {
                        $grade = "NA";
                    }
                ...
                // get total units
                $getTotalUnit = $this->pdo->prepare("SELECT SUM(unit) as total_unit FROM courses WHERE code IN (SELECT course_code FROM enrolled_courses WHERE enrollment_id IN (SELECT id FROM enrollments WHERE year_start = ? AND year_end = ? AND semester = ? AND student_id_no=?))");
                $getTotalUnit->execute([$yearStart, $yearEnd, $semester, $this->user['student_id_no']]);
                $totalUnit = $getTotalUnit->fetch()['total_unit'];

                $average = round($sum / $totalUnit,2);

                $grades .= "<li class='list-group-item d-flex align-items-center bg-dark-blue-accent justify-content-between'>" .
                        "<small class='text-light '>GWA</small>" .
                        "<small class='fw-bold text-light'>$average</small>" .
                        "</li>";
    ...
    function getEnrolledCourses(string $user_query): array
    {
        ...
        if (stristr($user_query, 'semester') && Session::hasSession($schoolYearSession)) {
            ...
            $get_enrolled_courses->execute([$this->user['student_id_no'], $yearStart, $yearEnd, $semester, $this->user['student_id_no']]);
            ...
            if (count($enrolled_courses) > 0) {
                foreach ($enrolled_courses as $key => $enrolled_course) {
                    ...
                    $enrolled_course_response .= "<li class='list-group-item '>" .
                        "<div class='row align-items-center px-2'" .
                        "<small class='text-secondary col-2'>$course_code</small>" .
                        "<small class='text-secondary col-1'>-</small>" .
                        "<small class='text-secondary col'>$course_name</small>" .
                        "</div>" .
                        "</li>";
                }
